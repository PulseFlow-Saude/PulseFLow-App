...
class _StepsHistoryScreenState extends State<StepsHistoryScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      _bootstrapOldestPatientRangeThenLoad('passos');
    });
  }

  /// Data inicial = registo mais antigo deste paciente na coleção; fim = hoje. O utilizador ajusta no picker.
  ...
  Future<void> _loadHealthData() async {
    ...
    try {
      ...
      // Filtra por período (dias civis inclusivos; aceita `data` ou `date` em vários formatos BSON)
      ...
      print('📊 [StepsHistory] Total de registros encontrados: ${filteredData.length}');
  ...
  Widget _buildAnalysis(Map<String, dynamic> stats) {
    ...
    // Análise baseada em recomendações de passos diários (10.000 passos)
    if (avg >= 10000) {
      analysis = 'Excelente! Você está atingindo a meta recomendada de 10.000 passos por dia.';
      analysisColor = Colors.green;
      analysisIcon = Icons.check_circle;
    } else if (avg >= 7500) {
      analysis = 'Bom! Você está próximo da meta de 10.000 passos. Continue assim!';
      analysisColor = Colors.lightGreen;
      analysisIcon = Icons.trending_up;
    } else if (avg >= 5000) {
      analysis = 'Moderado. Tente aumentar para pelo menos 7.500 passos por dia.';
      analysisColor = Colors.orange;
      analysisIcon = Icons.warning;
    } else {
      analysis = 'Baixo. A OMS recomenda pelo menos 10.000 passos por dia para adultos.';
